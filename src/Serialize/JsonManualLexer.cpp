...
{
    void decodeUnicode(ParserInterface& i, std::string& reply);
    void decodeSurrogatePairs(long unicodeValue, ParserInterface& i, std::string& reply);
    long getUnicodeHex(ParserInterface& i);

    void checkBuffer(ParserInterface& i, std::string& reply)
    {
        reply.clear();
        while (i.ok())
        {
...
void JsonManualLexer::getStringInto(std::string& value)
{
    ...
    ThorsAnvil::Serializer::Unicode::checkBuffer(parser, value);
}
THORS_SERIALIZER_HEADER_ONLY_INCLUDE
